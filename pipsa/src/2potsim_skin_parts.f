...
c and over the conical part of the space.
c
c Input:
...
c   format of the file "parts" is: xr1,xr2,angle, where xr1 and xr2
c   are 3 coordinates (in &Aring;) of the beginning and the end of the
c   vector defining the direction of the conus and angle is angular
c   extent (in degrees, with 180.0 defining a whole space) of the conus.
...
c
CccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccC
      character*256 fpdb1,fpdb2,fgrd1,fgrd2,arg,fparts
      integer iform
      integer im, jm, km
...
c
 ...
 499  format('usage: 2potsim_skin_parts -g1 grd1.grd -g2 grd2.grd ',
     ,'-p1 pdb1.pdb -p2 pdb2.pdb -pa parts -pr probe -sk skin')
      call exit
      end if
c
      nargs=iargc()
...
c..These are OPLS parameters used in UHBD
     ...
     ,       (isk2a(i,j,k).eq.0)) goto 410
          ...
          zz=k*h+zo
          scp=
     +(xx-xr1(1))*(xr2(1)-xr1(1))+
     +(yy-xr1(2))*(xr2(2)-xr1(2))+
     +(zz-xr1(3))*(xr2(3)-xr1(3))
          pno= sqrt(
     +(xx-xr1(1))**2+                 
     +(yy-xr1(2))**2+                 
     +(zz-xr1(3))**2)                 
      cangle=scp/pno
          if(cangle.lt.scarr) then
          isk1a(i,j,k)=0
          isk2a(i,j,k)=0
          end if
 410  continue
