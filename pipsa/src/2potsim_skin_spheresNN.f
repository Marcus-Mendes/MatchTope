...
c amb  = average difference of the potentials a and b (a-b)/npoi
c ambl = log(sum[exp(a)]/sum[exp(b)])
c ambm = log(sum[exp(-a)]/sum[exp(-b)])
c ava   = average of potential a
c avb   = average of potential b
c
c  Output:
...
CccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccC
      ...
      integer iform
      integer im, jm, km
      dimension xsc(3,3999),ar(3999),rsc(3)
...
c
       ...
       nsphere=0
 71    continue
       read(iusphere,*,end=72,err=72) rsc,arr
       nsphere=nsphere+1
       ar(nsphere)=arr
        do m=1,3
        xsc(m,nsphere)=rsc(m)
...
ccc
 ...
 421  continue
     ...
     &                  xo,yo,zo,isk1a,isk2a,np1a,np2a,npoi,
     &                  aa,bb,ab,aa0,bb0,amb,ambl,ambm,ava,avb)
	  write(iulog,611) im,jm,km,h,xo,yo,zo
 ...
 611  format('grid dimensions:  ',i3,2('.',i3),'  spacing:',f8.3,/,
     ...
     ,,float(npoi)/sqrt(pnp12),np1a,np2a,npoi,amb,ambl,ambm,ava,avb
                 END DO
...
c
     ...
     &                  xo,yo,zo,isk1,isk2,np1a,np2a,npoi,
     &                  aa,bb,ab,aa0,bb0,amb,ambl,ambm,ava,avb)
...
c
      ...
      double precision dambma,dambmb,dava,davb
      common/ius/iulog
c
...
c ijk transform to n as n=i+(j-1)*im+(k-1)*im*jm
 ...
 421  continue
       ...
       ava=dava/dfloat(npoi)
       avb=davb/dfloat(npoi)
